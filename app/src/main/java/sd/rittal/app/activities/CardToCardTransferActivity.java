...
public class CardToCardTransferActivity extends AppCompatActivity implements FormSession,LoginSession {
    ...
    EditText et_card_pan,et_card_expiry_date,et_card_pin,et_other_card_pan,et_amount;
    Button btn_request;
    ...
    private void initiateLayout(){
        ...
        et_other_card_pan             = (EditText) findViewById(R.id.et_other_card_pan);
        ...
        toolbar.setTitle(getString(R.string.title_card_to_card_transfer));
...
//        createDialogWithoutDateField().show();
        ...
        btn_request.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                final String other_card_pan      = et_other_card_pan.getText().toString().trim();
                final String amount             = et_amount.getText().toString().trim();


                if(validation(pan,expiry_date,pin,other_card_pan,amount)){

                    new AlertDialog.Builder(activity)
                            .setIcon(R.drawable.ic_attention)
                            .setTitle(R.string.are_u_sure)
                            .setMessage(other_card_pan)
                            .setPositiveButton(activity.getResources().getString(R.string.yes),new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                    clearForm();
                                    doCardTransfer(pan,expiry_date,pin,other_card_pan,amount);
                                }
                            })
    ...
    private boolean validation(String pan , String expiry_date , String ipin , String other_card_pan , String amount){
        ...
        if(!expiry_date.equals("ACCC")) {
            ...
            if(pan.length() == 0 || expiry_date.length() == 0  || ipin.length() == 0 || other_card_pan.length() == 0|| amount.length() ==0){
                ...
                if(other_card_pan.length() == 0){rittal.rittalToast(getString(R.string.enter_other_card_pan),"1","w"); return false;}
                ...
                if(pan.length() == 16 || pan.length() == 19){
                    if(expiry_date.length() == 4){
                        if(ipin.length() == 4){
                            if(other_card_pan.length() == 16 || other_card_pan.length() == 19){
                                return true;
                            }else{
            ...
            if( other_card_pan.length() == 0|| amount.length() ==0){
                if(other_card_pan.length() == 0){rittal.rittalToast(getString(R.string.enter_other_card_pan),"1","w"); return false;}
                if(amount.length() == 0){rittal.rittalToast(getString(R.string.enter_amount),"1","w"); return false;}
                return false;
            }else{
    ...
    private void doCardTransfer(String pan , final String exdate , String ipin, String other_card, String amount) {
...
{"responseMessage":"Approved","responseMessageArabic":null,"responseCode":0,"responseStatus":"Successful","pubKeyValue":null,"tranDateTime":"180319142527","creationDate":null,"panCategory":null,"balance":"18.41","payeesCount":0,"payeesList":null,"billInfo":null,"RespPAN":null,"RespExpDate":null,"RespIPIN":null,"regType":null,"financialInstitutionId":null,"respUserName":null,"respUserPassword":null,"RespVoucherNumber":null,"voucherCode":null,"serviceInfo":null,"originalTransaction":null,"originalTranType":null,"originalTranUUID":null,"respUUID":null,"resp_acqTranFee":0,"resp_issuerTranFee":0,"resp_fromAccount":null,"resp_accountCurrency":null,"resp_tranAmount":0}
 */
